...
- **Auto-detect speakers (experimental)** — with the Whisper engine, an on-device voice-embedding model (sherpa-onnx) groups transcript segments by who's talking, labeling unnamed voices "Speaker 1", "Speaker 2"…; tap any auto-labeled segment and choose "Name Speaker N everywhere…" to rename the whole voice at once. Enable in Settings under the Whisper section and download a speaker model (one-time, ~30–40 MB). Manual tags always win over auto labels. (Design: DIARIZATION_PLAN.md.)
