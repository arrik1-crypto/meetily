...
- **Auto-detect speakers (experimental)** — with the Whisper engine, an on-device voice-embedding model (sherpa-onnx) groups transcript segments by who's talking, labeling unnamed voices "Speaker 1", "Speaker 2"…; tap any auto-labeled segment and choose "Name Speaker N everywhere…" to rename the whole voice at once. Enable in Settings under the Whisper section and download a speaker model (one-time, 26–109 MB: WeSpeaker ResNet34 is the recommended English default; the deeper WeSpeaker ResNet293 and ResNet152 are the most accurate; TitaNet large/small and CAM++ English+Chinese are also available). Saved voice profiles keep their enrollment audio, so when you switch speaker models they convert automatically on the next recording or import; only profiles saved without audio (from a live meeting's "Save voice", or before this feature) need re-enrolling — the app warns you which is which before switching. Manual tags always win over auto labels. (Design: DIARIZATION_PLAN.md.)
...
- **Summary templates** — choose the summary's shape when generating: General minutes, Action items, Decisions & open questions, Standup, Sales call report, 1:1, Job interview, User research, Retrospective, Project status, Follow-up email, plus domain shapes — **Clinical visit note (SOAP)**, **Patient consult recap**, **Legal consultation memo**, and **Case/matter notes** (all framed as drafts for professional review) — and your own **custom templates** (name + free-form instructions, saved on-device, managed from the picker's "Custom…" button). A **Brief / Standard / Detailed** toggle in the picker sets how deep any template goes, recurring meetings **remember their template per series** (your standup preselects Standup, your sales calls preselect the call report), and every template — custom ones included — now runs with shared output rules: verbatim names/dates/amounts, no invented facts, no filler sections, no preamble.
...
- **On-device AI (embedded)** — Settings → AI summaries → engine "On-device" runs an LLM (llama.cpp, GGUF) entirely on the phone: summaries, Ask, digest, topics, and speaker suggestions with zero network involvement, ever. Twelve models from 0.5 GB to 5.7 GB: Qwen 2.5 0.5B/1.5B and Llama 3.2 3B for every phone; Gemma 3 1B/4B, Gemma 4 E2B/E4B, Mistral 7B, Llama 3.1 8B, and Qwen 3.5 9B for phones with the RAM to match (labels state the requirement — the 7-9B tier wants a 12-16 GB flagship); plus two domain specialists — **MedGemma 4B** (Google's medical Gemma tune, strong on clinical vocabulary) and **SaulLM 7B** (legal, trained on case law/contracts/legislation). Domain summaries are drafts, not clinical or legal documents. Reasoning models' think-aloud blocks are stripped automatically. The attention cache runs quantized (q8_0 + flash attention, with automatic fp16 fallback), freeing ~0.5 GB on the 7-9B tier so big models survive longer on 12 GB phones. Slower than a server; long meetings are handled with a map-reduce pass — the transcript is condensed section-by-section on-device, then summarized from the ordered notes, so hour-long meetings get full coverage (at the cost of a few extra minutes of compute). Quality sits below big cloud models, privacy sits above everything.
- **Summaries**, two ways:
...
- **Custom vocabulary** — list names, acronyms, and jargon in Settings and Whisper is biased toward them (its initial prompt) for all new recordings and imports. One-tap **medical and legal starter glossaries** seed the list with commonly fumbled domain terms (drug names, procedural Latin…) — prune and add your own; your names and jargon matter most.
...
- **Download queue** — model downloads run in the background (foreground service + wakelock) and further picks queue up: tap a transcription model, a speaker model, and an on-device LLM back-to-back and they download consecutively under one notification (with a per-model summary at the end). Cancel stops the current download and clears the queue.
