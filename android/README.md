...
- **Nocturne design** — the app wears the Nocturne identity from a Claude Design handoff: Inter type, a deep indigo-violet dark theme (the hero) and a "Daylight blue" light theme, depth via glow and layering. Home centers on a record orb ("Tap to record — nothing leaves this phone") over a gradient hairline and RECENT MEETINGS cards with icon wells; recording gets a radial ground, a 44sp light-weight timer, and outlined pill controls; speakers appear as tinted chips throughout. A sun/moon toggle in the home header flips themes instantly (System/Light/Dark also in Settings).
