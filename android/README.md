...
- **Broadsheet design** — the app wears a newsprint identity from a Claude Design handoff: Source Serif 4 throughout (the serif is the chrome), paper-white ground with near-black ink, cyan as the one interactive color and magenta reserved for the recording state, no cards or boxes — hierarchy is type and whitespace. The home screen is a front page (centered masthead, thick-thin dateline rail, meetings grouped under day datelines with times in the margin); recording is "on air" with a big serif timer and a magenta pill. A System / Light / Dark toggle switches between the day and night editions.
...
- **Quick-record entry points** — long-press the launcher icon for Record / Import shortcuts, or add the "Record meeting" Quick Settings tile.
